...
Note that we use the original Aschbacher classes \CC1 to \CC9 instead of
our new classes \DD1 to \DD9 in this chapter, because this makes the text
readable independently from this book. For the purposes of finding
reductions for groups in the classes \DD3 and
\DD5 the methods below for \CC3 and \CC5 can be applied without change.
...
The classes $\CC1, \ldots, \CC8$ are treated by reducing to 
...
we either find a proper nontrivial homomorphism from $G$ to a permutation, matrix or projective group, or an isomorphism writing $G$ or $\overline{G}$ over a smaller field, or in a smaller dimension. 
  In addition, for some groups in the classes \CC2, \CC4 or \CC6,
...
In Section~\ref{realisesubfield} we explain an 
...
%Jon Carlson is grateful to the RWTH in Aachen for their kind 
...
module $V = \F_q^d$.
%and that $\overline{G}$ acts irreducibly
...
if there is a divisor $e$ of $d$ with $1 < e < d$, and 
an $\F_q$-vector space identification between
...
algorithmic results of this chapter.

\begin{Theo} [Main Theorem]
Let $G = \left< g_1, \ldots, g_m \right> \leq \GL(d, q)$ or 
$\overline{G} = \left< \bar g_1, \ldots, \bar g_m \right>\leq \PGL(d, q)$ 
be an absolutely 
...
algorithm to find a nontrivial
reduction of $G$ or $\overline{G}$, respectively.
\end{Theo}
...
\item the order of $G$ is a divisor of $c^{l}z$, where $l = |\left\{ i \mid
g_i \notin Z\right\}|$.
\end{enumerate}
\end{Prop}
...
Lemma~\ref{lem:facts}.6 it follows that $g_i^c$ is in the centre of $G$ 
...
Let
$\rho(g) \in \GL(n,k)$ have all eigenvalues in  $k$.
...
Let $r$ be a prime and $G$ an  $r$-group %such that $G' \leq Z(G)$
...
The group $G$ also contains a central element of order $r$, which is mapped to a 
scalar. This shows that the field $k$ contains primitive $r$-th roots of
unity.  Hence all of the eigenvalues 
...
that the elements of $G$ permute the eigenspaces. 
...
for $H$ to have the same submodule structure and endomorphism ring as $N$ 
...
\begin{Lemm} %[{See \cite[Corollary 2.2]{babai95}}]
    ...
    previous $X_j$ (but the distribution of $X_i$ may depend on the 
    outcome of the $X_j$ for $j < i$). 
...
proved for the case $G$ transitive. Note that the hypotheses here are       
slightly more general than in \cite[2.3.7]{Ser}, where $G$ is given as a 
group of permutations. Our Proposition~\ref{prop:orbits}
can for example be applied to any finite group equipped with a   
permutation action.                                                     
...
are proper subsets of orbits of $G_{i}$. Thus, with probability at least
$1/3$, 
...
%\pagebreak
We now apply the previous proposition to matrix groups by considering their
action on vectors.
...
%multiplication, except for $\{0\}$, 
so the first claim follows. 

For the second claim, let $G$ be irreducible and $\End_{\F_qG}(V) = \F_{q^f}$.
Let $H_0$ be generated 
by $O(\log \delta^{-1} + d \log q)$ random 
elements of $G$, so that $H_0$ is irreducible 
with probability $1-\delta/2$ (adjusting the constant
in the capital $O$). Let $t = c \log d$ 
...
with $x$. If $[h_i, x] \neq 1$ 
then $N_i$ is a proper divisor of $N_{i-1}$ so the claim follows. 
...
by Lemma~\ref{babailemma} with $p = 1/2$, $t = c \log d$ and $\epsilon =
1 - 2/(c \log 2)$, % \cite[Corollary 2.2]{babai95}
 \[\Pro(\sum_{i = 1}^t Y_i \leq \log d) \leq e^{-\frac{1}{4}(1-
\frac{2}{c \log 2})^2 c \log d} \leq e^{-\frac{3}{16}c \log d}\] since
$c \geq 22$.
...
\section{Writing $G$ over a smaller field}
...
This is also called the \emph{$F$-enveloping algebra of $G$}.
We denote the prime field of $K$ by $K_0$.
...
Let $E := \End_{F[G]}(V) = \End_{FG}(V)$ with  
$e = [E:F]$ and $d' = d/e$. Identify $V$ with 
$E^{1 \times d'}$ %and $G \le GL(d',E) \le GL(d,F)$, 
...
\end{Prop}
\begin{proof}
Clearly $e = 1$ and  $E=F$ if and only if $V$ is absolutely irreducible.
...
an $F$-basis of $V$. 
This choice of basis fixes an embedding
$E^{d' \times d'} \subseteq F^{d \times d}$. 
Since the action of $G$ on $V$ is $E$-linear, 
we may assume that
...
The field $L$ does not need to contain $F$. However, the elements of $B$ corresponding
...
additionally  that 
there is no proper subfield $D$ of $F$ such that there exists $t \in \GL(d,F)$
...
smallest subfield of $F$ over which $G$ can be written, so
$L = F = E$ as required.

Now consider the case $F < E$
...
We claim that $G$ can be written over $D$. 
...
However, by our assumption that $F$ is the smallest subfield of
$F$ over which $G$ can be written, we get $D = F \cap L = F$ and thus
$F \le L$. From $F \cdot L^{d' \times d'} = E^{d' \times d'}$ follows now
immediately $L=E$.

Since we have proved $L=E$ in both cases and we already know that $F_0[G]$
is isomorphic to $L^{d' \times d'}$ as $F_0$-algebras and contained in
$F[G] = E^{d' \times d'}$ the proposition follows.
\end{proof}
...
hold for some subfield $F$ of $K$. We now develop some theory which leads to 
...
$V$ is isomorphic to $K \otimes_F \tilde{V}$ as $KG$-modules, then
...
Let $F$ be a subfield of $K$ such that the equivalent statements in
Lemma~\ref{extsca} hold and let $\tilde{V}$ be as above.
Then the $F$-dimension of\/ $\End_{FG}(\tilde{V})$ is equal to 
$e = \dim_F(\End_{KG}(V))$.
\end{Lemm}
...
for all $1 \le j \le d$ and all $g \in G$ and by the uniqueness 
above this in turn is equivalent 
to $\psi_i(m_j g)-\psi_i(m_j)g=0$ for all $i$, $j$, and $g$. This
proves that $\End_{KG}(V) \cong K \otimes_{F} \End_{FG}(\tilde{V})$.

Now the $F$-dimension of $K\otimes_{F} \End_{FG}(\tilde{V})$ equals
$f \dim_F(\End_{FG}(\tilde{V}))$ and the $F$-dimension of $\End_{KG}(V)$ is $e f$,
...
\label{lindep}
Let $F$ be a subfield of $K$ such that the equivalent statements in
Lemma~\ref{extsca} hold and let $\tilde{V}$ and $B = \{ b_1, \ldots, b_f\}$
be as above.
Let $w \in \tilde{V}$ and $x_1, \ldots, x_k \in FG$. The set of vectors
$\{\sum_{i = 1}^f b_i w x_j \ | \ 1 \leq j \leq k\}$ is linearly
independent over $K$ if and only if it is linearly independent over
$F$.
\end{Lemm}
...
been multiplied by the \emph{same} element $a^{-1} \in K$, the same statement
is true over $F$.
...
such that every $F$-basis of $\tilde{V}$ is mapped to a $K$-basis of $V$.
The representing matrices with respect to such a basis
...
\smallskip
\begin{proof}
The kernel $\ker_V(c)$ is $E$-invariant and thus
...
Let again $e := \dim_K(\End_{KG}(V))$ be the degree of the
splitting field. We assume that $e$ is already computed.
...
Let $F$ be the smallest subfield of $K$ for which there is an $r \in \GL(d,K)$
such that $r^{-1}Gr \leq \GL(d,F)$ (see Theorem~\ref{charsmallest}), 
then $t^{-1}Gt \leq \GL(d,F)$ as well. 
...
\begin{proof}
Let $B = \{ b_1, \ldots, b_f \}$ and $\tilde{V}$ be as in the paragraph
before Lemma~\ref{degsplittingfield} and let $E := \End_{F[G^r]}(\F^{1
\times d})$, then by Lemma~\ref{degsplittingfield}, we have $e = [E:F]$
and know it in advance, since we know $\End_{KG}(V)$ by a MeatAxe run.
...
Note that we assume that  the isomorphism exists but do not yet
have it explicitly! 
...
but by Lemma~\ref{degsplittingfield}, $e = 
...
and stop if $\dim_K(\ker_V(c)) = e$. 
 The results in \cite{MeatAxeHoltRees} and \cite{IL}
    ...
    $d$ for the probability that a random element 
    $c \in K_0[G] = F_0[G] \cong F[G^t] \cong E^{(d/e) \times (d/e)}$ 
    (compare Theorem~\ref{charsmallest}) has
    $\dim_{K}(\ker_V(c)) \neq e$. Thus $\log_b \delta^{-1}$ 
    tries will succeed with probability at least $1 - \delta$.  
    Note that these arguments prove that such a $c \in K_0[G]$ actually
    exists!
...
We can now use the standard basis 
...
Note that the latter is a basis for the image of this $FG$-homomorphism, 
which is an $F$-subspace, and that these standard bases are defined using
$F$-linear independence. 
%which has $F$-dimension $d$. 

We do yet know $F$, so we cannot yet test for $F$-linear independence. 
...
mapped to $t$ by $\alpha$. 

This proves our claim that we in fact compute $SB(V,w,(g_1,\ldots,g_m))$
with testing for $F$-linear independence.

3. By the above arguments, the 
result $SB(V,w,(g_1, \ldots, g_m))$  is an $F$-basis of an $FG$-submodule
...
coefficients of $t^{-1}g_it$ for \mbox{$1 \leq i \leq m$}.
...
scalar, since $G$ modulo scalars is abelian.
Thus the methods of Section~\ref{subsec:scalars} apply with $G'$ known
to be scalar. If any of them is found to be non-scalar we proceed to
generate $H$ as described below.
...
sum of isomorphic irreducible $N$-submodules of dimension greater than $1$, so that
in particular $N$ is non-scalar.
\item $N$ is reducible, and $V$ splits into a direct sum of isomorphic
$1$-dimensional submodules, so that $N$ is scalar.
\end{enumerate}
...
We can therefore assume in the case of this section that $G$
lies in class \CC5.
...
Let $H = \langle n_1, \ldots, n_s \rangle \le N \unlhd G$ with $N \le G'$
and let $H$ be known (by a MeatAxe run) to be absolutely irreducible. 
Then in $O(sd^3 \log \delta^{-1} + md^3)$ field operations
we can construct a homomorphism from $G$ to\/ $\PGL(d, q_0)$ for minimal $q_0$
 or prove that $G$ and
...
\begin{proof}
Since $H$ is absolutely irreducible we use the methods of
Section~\ref{realisesubfield} to find a matrix $t$ such that $t^{-1}Ht
\leq \GL(d, q')$
with $\delta$ as an upper bound for the failure probability.
This automatically finds the smallest prime power $q'$ with this       
property. Notice that the vector chosen in the kernel of $c$
by the standard basis method is unique up to multiplication 
by elements of $\End_{\F_q H}(V) = \F_q$. From this point on 
...
In deterministic $O(d^4\log q + m d^3)$ field operations we can construct
...
The matrix $C$ does not need to generate the
multiplicative group of $\F_{q^e}$, but its powers $C^0, C^1, \ldots,
C^{e-1}$ are $\F_q$-linearly independent. 

As shown in Proposition~\ref{propsemilinear},
the group $G$ acts by conjugation as field automorphisms on $\F_{q^e}
= \End_{\F_q N}(V)$ and thus on the group $\left< C \right>$.
We can immediately
...
Computing this action provides a homomorphism from $G$ to the cyclic group of order
$e$, because the above mentioned matrices are the possible images
of $C$ under automorphisms of $\F_{q^e}$.
...
vC^{e-1}$ by repeatedly multiplying with $C$. This spinning algorithm gives us a
base change to an $\F_{q^e}$-adapted basis.
It needs at most $O(md^3)$ field
operations.
...
of $G$ need not
act as field automorphisms on $\F_{q^e}$, which we notice during the
above computation. However, if $G$ is contained in 
\CC3 then they
...
is an $N$-submodule. 
\begin{Lemm} [Finding an irreducible module] \label{lem:find_irred}
...
\begin{Theo} [Construction of a block action] \label{thm:imp_reduct}
Let $G = \langle g_1, \ldots, g_m \rangle \leq \GL(d, q)$ or its
...
permutation group with kernel the pointwise stabiliser of the set of
homogeneous components of $V|_{\F_q N}$
...
of failure $\delta$. This subfactor is an irreducible module $\tilde S$ and we can now
apply the isomorphism testing
procedure described in Theorem~\ref{isotest} once to give a homomorphism of
$\tilde S$ into $V|_{\F_q N}$ and thus an irreducible submodule
$S$ with $O(sd^3)$ field operations. Note that when we
proved the final subfactor in the procedure described in Lemma~\ref{lem:find_irred}
to be irreducible,  we constructed the algebra
...
Since $\F_q$-scalars act trivially on the set of homogeneous
...
irreducible $H$-submodule $S$ and run the\linebreak \mbox{{\sc MinBlocks}} procedure. If
...
Let $N = \langle n_1, \ldots, n_s \rangle \unlhd G$ be reducible,
with a single homogeneous component of dimension $n > 1$, and let an
irreducible $N$-submodule $S$ be given.
In deterministic $O((s+m)d^3)$ field operations we can construct a proper nontrivial 
homomorphism from $G$ into\/ $\PGL(d/n, q^e)$ for $e$ the $\F_q$-dimension
of\/ $\End_{\F_q N}(S)$.
\end{Theo}
...
\cdot \left[ \begin{array}{c|c|c|c}
      D^{g^{-1}}(h) & 0   & \cdots &      0    \\ \hline
         0   &D^{g^{-1}}(h)&\cdots &      0    \\ \hline
      \vdots  & \vdots  & \ddots & \vdots    \\ \hline
         0    &    0    & \cdots& D^{g^{-1}}(h) \end{array} \right] \\
 &=& \left[ \begin{array}{c|c|c|c}
      D(h)    & 0       & \cdots &     0    \\ \hline
         0    &D(h)     &\cdots &      0    \\ \hline
      \vdots  & \vdots  & \ddots & \vdots    \\ \hline
...
representing $h$ on the module $S$ and $D^{g^{-1}}(h) = D(g^{-1}hg)$ is the
same representation twisted by the element $g^{-1}$. By the block diagonal
structure of the matrices in $N$ we get 
$g_{i,j} \cdot D^{g^{-1}}(h) = D(h) \cdot g_{i,j}$ for all $i$ and $j$ and 
all $h \in N$.

But by hypothesis, the matrix representations $D$ and $D^{g^{-1}}$ of $N$
are isomorphic. Thus there is a nonzero matrix $T \in \F_q^{n \times n}$ with
$T \cdot D^{g^{-1}}(h) = D(h) \cdot T$ for all $h \in N$. By Schur's lemma and
since the representation $D$ is irreducible,
the matrix $T$ is invertible and unique 
up to left multiplication by an element of $C_{\GL(n,q)}(D(N))$, which
...
$g_{i,j} = e_{i,j} \cdot T$. Thus we have shown that with respect to
...
generators of $G$ might not be Kronecker products after a corresponding
...
into the multiplicative group of the field that does
not necessarily need to correspond to an imprimitive 
...
\begin{proof}
We are given a non-scalar generator $g_i$, such that all commutators of 
it with all other generators are scalar matrices. Thus $g_i$ is central in
$G$ modulo scalars, thus the commutators of $g_i$ with all elements of $G$ are
scalar. Therefore, the map $\psi_{g_i} : G \to \F_q, g \mapsto [g,g_i]$ is
a group homomorphism into the scalar matrices. This is proved exactly as
Lemma~\ref{lem:facts}.5. 

The kernel of $\psi_{g_i}$ is $C_G(g_i)$. Since $g_i$ is non-central, $\psi_{g_i}$ 
is nontrivial.
...
Since $g_i \in C_G(g_i)$, the kernel is
not an absolutely irreducible group and may even not be irreducible. 
...
operations to get generators for the image. Some of them could be trivial,
so we get at most $m$ generators.
Multiplying each generator of $G$ by the inverse of its image 
in $O(m d^3)$ field operations will produce at most $m$ 
generators for the kernel, ignoring again trivial ones.
\end{proof}
...
element both in the group generated by a set $X$ of matrices, and in its 
normal closure  in
 a  group $G \le \GL(d,q)$, in $O(d^3)$ elementary field
operations.
Assume further that we can generate a sufficiently evenly distributed
random element in a matrix algebra $\mathcal{A} \le \F_q^{d \times d}$
generated by a set $X$ of matrices
in $O(d^3)$ elementary field operations.
\end{Hyp}

Note that this assumption is valid in practice by using Product
Replacement and \textsc{Rattle}%
\index{Rattle@\textsc{Rattle}}%
\linebreak (see \cite{ProdRep, LGMurray, LGO97}) 
 for groups and normal closures, and by taking random 
linear combinations of random products of generators for algebras,
at least after an initialisation phase.
However, even using such techniques,
these assumptions are not proven to hold in general!
