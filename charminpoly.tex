...
by $\{v^{(1)},\dots,v^{(m)}\}$, that is, the smallest $M$-invariant
...
\medskip
For the case of large $k$, we may use the procedure suggested in Remark\ref{algdeterm} as a verification algorithm, at a cost of $O(k^2n^2)$ field operations. Two alternative verification procedures are given below. The first involving evaluation on vectors is given in Proposition~\ref{eval}, and the second using null space computations is given in Proposition~\ref{propverify}.
