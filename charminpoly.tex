...
%Apart from these applications it has
...
which a basis is known. Throughout the chapter logarithms are always taken 
to base $2$.
...
$O\big((\log_2 \,\ve^{-1})(\log_2 n)(\xi_n $ $+ n^2\log_2^3 q) +n^3\log_2^2q\big)$& 
...
$M$ to the vector repeatedly computing a sequence 
...
\begin{Prop}[Probability that an lcm of order polynomials is
equal to $\mu_{M,V}$]
\label{ProbAllMult}
...
\[ 6n^3 + {\rm fact}(n,q) + 
  2 \lceil\frac{\log_2 n-\log_2 \epsilon}{\log_2 q}\rceil^2 \cdot n^2 
\]
...
upper bound in Proposition~{\rm\ref{propminpoly}} of $O(n^3\log_2^3 q)$
...
\[
u_0:=\lceil \frac{\log_2 n + \log_2 (\epsilon^{-1})}{\log_2 q}\rceil
\]
...
\[ 
%\sum_{i=1}^t \left( 2r_in^3 + 2\log_2(f_i) \, n^3 + n^3 \right) =
 n^3 \cdot \sum_{i=1}^t \left( 2r_i + 2\lceil \log_2 f_i \rceil+1 \right).
\] 
\end{Prop}
...
with at most $2n^3\lceil \log_2 f_i \rceil$ elementary field operations, and 
