...
Storjohann's earlier deterministic algorithm (see \cite{Stor98}) uses classical 
...
\REPEAT
    ...
    \COMMENT{this is the $q$ from Lemma~\ref{ordpolcyclic} for computing
    $\ord_{M'}(v+W'_{i-1})$}
    \IF {$h \neq 0$}
    \STATE $\hat g := p^{(i)}/\gcd(h,p^{(i)})$ \hspace*{1cm} \COMMENT{factorised}
        ...
        \COMMENT{$g=\hat g$, but the latter only exists in factorised form}
        \IF {$i > 1$}
...
\begin{Rem}[Modification to get a deterministic version]
    \label{algdeterm}
...
$1-\varepsilon$.
  ...
  From the discussion at the beginning of Section~\ref{minpoly},
...
\begin{Rem}[Performance in practice]
...
\begin{Rem}[Further optimisations]
...
%Project and Federation Fellowship Grant of Cheryl Praeger funded by the
