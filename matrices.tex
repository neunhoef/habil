...
the Table~\ref{MatMulWinograd2}). The situation for the base field 
...
to use the Winograd at all. For smaller matrices in the range $n=2000$
to $n=3000$ on the other hand it seems worthwhile to use Winograd once
which brings the pieces into the second level cache. For $n=5000$ in
...
We conclude this section with Figures~\ref{matmulf2} amd
...
All modern microprocessors have builtin caches, most of them more than
...
standalone implementation of basic linear algebra routines using
...
in at most $2\lceil\log(m)\rceil$ multiplications in $R$.
\end{Lemm}
\proofbeg
Let $l := \lfloor\log(m)\rfloor$ and write $m$ in its binary expansion 
