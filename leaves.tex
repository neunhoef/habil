...
means of references into the literature. A more detailed account of
...
itself not necessarily simple, after all $G$ is only ``almost simple''.
...
\cite[5.3]{RecogTensInd} to estimate the order of an element in a
factor group if the group and the normal subgroup are given.
This technique usually produces generators for $N$ efficiently.
...
\DD9 one would have to be able to do constructive recognition for all
...
irreducible.  We will ignore this problem here.

We next try to give an overview over the known techniques for
...
\cite{primpowgraphs}, a newer one in \cite{findingcharlie} and there
