...
``almost-simple''). 
Thus the $G$-module $V$ gives rise to an
absolutely irreducible projective representation for $T$.}
...
$\F_{q^s}$-linear. Thus $G$ lies in class \CC3 with trivial Galois
automorphisms.
...
Assume first that $G/Z$ is a simple group. Then either $G/Z$ is one of the
classical simple groups and $G$ lies in \CC8 or $G$ lies in \CC9 with
$S=T \cong G/Z$ since $V$ gives rise to an absolutely irreducible
projective representation of the finite simple group $G/Z$.

Assume from now on that $G/Z$ is not simple, let $\bar N$ be a non-trivial minimal 
normal subgroup of $G/Z$ and $N$ be the corresponding non-scalar normal
subgroup of $G$ with $Z < N \triangleleft G$.
...
if $W$ absolutely irreducible, then \CC4 tensor

otherwise tensor dec. with one factor over extension field

or: if $W$ not absolutely irreducible, then \CC3???

$W = V$:

if $W$ not absolutely irreducible, then \CC5???

if $W$ absolutely irreducible:
