...
be a member of \DD8 or \DD9. In that case the constructive recognition problem
has to be
...
together with references into the literature. Furthermore, we present new 
...
\label{alginvjump}
\begin{algorithmic}
\STATE \textbf{Input:} A group $G \le \GL(n,q)$, an involution
...
number $m$ of generators of $G$ and $\log q$. The major question to be
resolved is whether there are groups in \DD2 or \DD4, for which it
does not work well or not at all.
\end{Expl}

\smallskip
We conclude this section with a few references into the existing
...
\cite{tensprodproj} and \cite{recogtensprod}.
...
case. To some extend it seems to work even better in this case, since the 
...
second is that even if we assume this, we have find the action of $G/N$ on
...
statement ``Perform check for \DD7'' in a situation where we have produced
...
This whole prodecure finds the action of $G$ on the direct factors of $N/Z$
...
\cite{recogtensprod}. Note that this covers the more general case of a
