...
\medskip
We proceed with the description of the Aschbacher classes. Throughout,
we denote for a subgroup $G < \GG$ its subgroup of scalar matrices
by $Z$, that is, $Z := Z(\GL(n,q)) \cap G$. For each class, we also
give the structure of the maximal members, this information is from
...
\desc{
A group $G \le \GG$ is a member of \CC1 if there is a subspace
...
\desc{
A group $G \le \GG$ is a member of \CC2 if there is a decomposition
...
\desc{
A group $G \le \GG$ lies in class \CC3 if there is a finite field
...
\desc{
A group $G \le \GG$ lies in class \CC4 if there is a decomposition
...
\desc{
A group $G \le \GG$ lies in class \CC5 if there exists a proper subfield 
...
\cdot \F_q^*$ where $[\F_q:\F_{q_0}]$ is a prime.

\subsection{Description of class \CC6: ``extraspecial''}
\label{descC6}

\desc{
A group $G \le \GG$ lies in class \CC6 if $n=r^m$ for a prime $r$ and
...
$\F_r$-vectorspace $E/Z(E)$ of dimension $2m$ is irreducible. }

\stru
Every such group is conjugate in $\GG$ to a subgroup of one of the following
groups: $r^{1+2m}.\Sp(2m,r)$ or $2_+^{1+2m}.O^+(2m,2)$ or
$2_-^{1+2m}.O^-(2m,2)$ or $(4 \circ 2^{1+2m}).\Sp(2m,2)$ 
(see \cite[Formula (4.6.1)]{kleilieb}).

\subsection{Description of class \CC7: ``tensor-induced''}
\label{descC7}

\desc{
A group $G \le \GG$ lies in class \CC7 if for some $k$ it has a normal 
...
\desc{
A group $G \le \GG$ lies in class \CC8 if $G/Z$ contains a classical simple
...
\stru
The maximal members in \CC8 are $\GL(n,q)$, $Sp(n,q)$, $U(n,q)$ and
...
\desc{
A group $G \le \GG$ lies in class \CC9, if there is a non-abelian, non-classical 
simple
...
\CC3 (see \ref{descC3}) with trivial Galois automorphisms.
...
$G$. Hence $G$ lies in \CC6 with odd $r$.
...
$E/Z(E)$ is irreducible and thus $G$ lies in \CC6 with $r=2$.
...
the tensor factors transitively. So $G$ lies in \CC7 
concluding case~(ii).
...
cannot be classical defined over a subfield because it is not in \CC5) and
$G$ lies in \CC8, or $G$ lies in \CC9. Note that the assumptions we
...
$G$ which is not realisable over a subfield (since $G$ is not \CC5). In
particular we can leave out finite classical groups in class \CC9.
\proofend
