...
modules that are not all isomorphic.
...
\DD4. At the end of this section we give references into the literature
to describe the current state of the art.

The descriptions of the classes \DD2 and \DD4 (and indeed \DD7 as we will
see in the next Section~\ref{solveD7}) have one property in common: they
...
the \DD7 case as well. We need a definition first.

\begin{Def}[Projective involution]
\index{Involution!projective|textbf}\index{projective Involution|textbf}
...
\index{Involution Jumper|textbf}
\label{alginvjump}
\begin{algorithmic}
\STATE \textbf{Input:} A group $G \le \GL(n,q)$, a projective involution
$x \in G$, an integer $t$, and a boolean $b$.
\STATE \textbf{Output:} A projective involution in $G$ or \textsc{Fail}.
...
element of $G$}
    \STATE $c := [x,y]$, and
    $o := \textsc{ProjectiveOrder}(c)$
    \IF {$o > 1$}
        \IF {$o$ is even}
            \STATE \textbf{Return} $c^{o/2}$
        \ELSIF {$b$}
            \STATE $z := y \cdot c^{(o-1)/2}$, and
            $o := \textsc{ProjectiveOrder}(z)$
            \IF {$o$ is even}
...
Let $G \le \GL(n,q)$ and $Z := G \cap Z(\GL(n,q))$ be the subgroup of scalar
matrices. If Algorithm~\ref{alginvjump} ``\textsc{InvolutionJumper}'' 
is called with $G$ and a projective
involution $x$, the return value $u$ is either a projective involution in $G$ with
$[x,u] \in Z$ or \textsc{Fail}. If the boolean argument $b$ is
\textsc{False}, then $u$ is contained in the group $[H,G]$ for
...
repeat some arguments from there: Thinking in $G/Z$ (indicated by bars), 
the element $\bar x$ is an
involution and for $\bar c := \bar x^{-1} \bar y^{-1} \bar x \bar y$ we have 
$\bar x \bar c = \bar c^{-1}\bar x$ and thus
$\bar x\bar c^k = \bar c^{-k}\bar x$ for all $k$. Thus, if $\bar c$ has even 
order $2k$, then
$\bar c^k$ is an involution commuting with $\bar x$ and $c^k$ lies in $[H,G]$ 
for every subgroup $H \le G$ with $x \in H$ by construction.

Now assume that $\bar c$ has odd order $2k+1$,
then $\bar x\bar y\bar c^k = \bar y\bar x\bar c\bar c^k = \bar y\bar
c^{-k-1}\bar x=\bar y\bar c^k\bar x$ and $\bar z := \bar y\bar c^k$ is an 
element in the 
involution
centraliser $C_{\bar G}(\bar x)$. In fact, Richard Parker has observed that
if $\bar y$ is
uniformly distributed in $\bar G$, then $\bar z$ is uniformly distributed in
$C_{\bar G}(\bar x)$ in this case. Thus, if the projective order of $z$ is even,
then Algorithm~\ref{alginvjump} ``\textsc{InvolutionJumper}'' 
returns a projective involution as well.

If the boolean argument $b$ is \textsc{False}, the odd order case is
ignored and therefore the statement of the proposition holds.
\proofend

Obviously, the Involution Jumper (Algorithm~\ref{alginvjump}) needs a
projective involution to start with. To this end we use a very simple minded
algorithm to find one, which is given as Algorithm~\ref{alginvmaker}
``\textsc{InvolutionMaker}''.

\begin{algorithm}
\caption{$\quad$ \sc InvolutionMaker}
\index{Involution Maker|textbf}
\label{alginvmaker}
\begin{algorithmic}
\STATE \textbf{Input:} A group $G \le \GL(n,q)$ and an integer $t$.
\STATE \textbf{Output:} A projective involution in $G$ or \textsc{Fail}.
...
element of $G$}
    \STATE $o := \textsc{ProjectiveOrder}(x)$
    \IF {$o$ is even}
...
Let $G \le \GL(n,q)$ be a group and $Z := G \cap Z(\GL(n,q))$ be the
subgroup of scalar matrices. Then the return value $u$ of 
Algorithm~\ref{alginvmaker} ``\textsc{InvolutionMaker}'' 
called with the group $G$ is either
a projective involution or \textsc{Fail}. The latter result occurs only if
...
contains already a line to check for \DD7 which we will explain in
...
\smallskip
\STATE $x := \textsc{InvolutionMaker}(G)$
\IF {$x = \textsc{Fail}$}
    \STATE \textbf{Return} \textsc{Fail}
\ENDIF
\STATE $i := 0$
...
\cite{LGMurray, LGO97}). In all cases of groups $G$ in \DD2 or \DD4 we
...
respect to visiting each vertex. Even better, the vertices representing
...
number of generators of $G$, and $\log q$.
\end{Expl}
