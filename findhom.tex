...
\desc{
A group $G \le \GG$ is a member of \DD2 if the natural module $V$ is
irreducible and there is a decomposition
...
\diffasch2
We include in \DD2 the subgroups of the members of \CC2 that permute
the direct summands transitively, but we exclude all groups acting
reducibly.
...
\desc{
A group $G \le \GG$ lies in class \DD3 if the natural module $V$ is
...
\smallskip
\stru
A group lies in \DD3 if and only if it acts irreducibly and is conjugate
in $\GG$ to a subgroup of $\GGL(n/s,q^s)$ for some prime $s \mid
n$, realised as $(n \times n)$-matrices over $\F_q$ by choosing an
$\F_q$-basis of $\F_{q^s}$.
...
%%% }
\desc{
A group $G \le \GG$ lies in class \DD4 if the natural module $V$ is
...
In this case, there is an absolutely irreducible projective
representation of $G/N$ on an $\F_q$-vector space $U$ of dimension $k$ and $V$ is
isomorphic as an $\F_q$-vector space to the tensor product $W
\otimes_{\F_q} U$.
}

\diffasch4
The class \DD4 is quite different from \CC4.
