...
Therefore the commutator subgroup $N'$ of $N$ is contained in $Z$ and
we recall that $Z$ is the center of $N$ since $W = V|_N$ is absolutely
...
Sylow subgroups of $N$ are scalar since $N/Z$ is elementary abelian.
Thus the module $V|_R$ is absolutely irreducible and $R$ is not
abelian. Since $R$ is characteristic in $N$ it is normal in $G$. 
For $x,y \in R$, we have $1 = [x^r,y] = [x,y]^r$ since $x^r \in Z(R)$.
Therefore $R'=N'$ has exponent $r$ and because it is contained in the
cyclic group $Z(R) \le Z$ we have $|R'| = r = |N'|$.

Assume first that $r$ is odd. We then claim that $R$ is an extraspecial group
with exponent $r$. For $x,y \in R$ we have $(xy)^r =
x^ry^r[y,x]^{r(r-1)/2} = x^ry^r$ since $R'$ has exponent $r$, thus the
elements of $N$ whose order is not divisible by $r^2$ form a characteristic 
subgroup of $N$ and thus a normal subgroup of $G$.

and thus $k$ is even and $n=r^{k/2}$. 

Consider now $r=2$. We then claim that $R$ is the central product of
an extraspecial $2$-group with a cyclic group of order $4$ and thus
...
